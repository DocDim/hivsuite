			...
			<div id="content" class="column span9">
						...
						<?php if(checkAdmin() || clinicien() || pharmacien() || nurse()) {
						 ...
						 if(isset($_POST['doSubmit']) and($_POST['doSubmit'] == 'Modifier'))
							...
							$req=mysqli_query($link, "UPDATE `dhappdatabase`.`dispensation` 
										SET `dataOfPickup`='$post[dataOfPickup]', `nextPickupDate`='$post[nextPickupDate]', `regimen`='$post[regimen]', `nbBox`='$post[nbBox]'
										WHERE id='$post[id]'");
							if (!$req) {
								echo $serialNumber;
							...
							if (!$req1) {
								...
								die(mysqli_error($req1));
								  ...
								  <table width="80%" border="0" cellpadding="5" cellspacing="2" class="myaccount">
									<tr>
									  <td><form name="dispensation_form" method="post" action="dispensation_update_form.php">
