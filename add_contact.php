			...
			<div id="content" class="column span9">
						...
						<?php if (checkAdmin()) {
						  ...
						  if(@$_POST['doSubmit'] == 'Save'){
								$number=str_replace(' ','',$post[number]);
								{mysql_query("INSERT INTO pbk (`GroupID`, `Name`, `Number`)
								VALUES ('$post[groupID]','$post[name]','$number')") or die(mysql_error()); 
								}	
						  ...
						  <h3 class="titlehdr">Add New Contact</h3>
						  <?php	
								if (isset($_GET['msg'])) {
						  ...
						  <table width="80%" border="0" cellpadding="5" cellspacing="2" class="myaccount">
							<tr>
							  <td><form name="form1" method="post" action="add_contact.php">
								  <table>
									<tr>
										<td></td>
										<td></td>
									...
									<tr>
										...
										<?php while ($rrows = mysql_fetch_array($rs_results)) {?>
											  <option  value="<?php echo $rrows['ID']; ?>"><?php echo $rrows['Name']; ?></option>
										<?php } ?>
									...
									<tr>
										<td>Phone number </td>
										<td><input name="number" type="text" id="number" placeholder="Phone Number"></td>
									</tr>
