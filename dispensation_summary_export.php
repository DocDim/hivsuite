...
foreach($_POST as $key => $value) {
	...
	$excel ="Code \t TARV Code \t Initiation Date \t Sex \t DoB \t Age \t Prof \t Statut \t Jan \t Fev \t Mar \t Apr \t May \t Jun \t Jul \t Aug \t Sept \t Oct \t Nov \t Dec \n";	
	...
	while ($rrows = mysqli_fetch_array($rs_total)) {									
										...
										$req2="SELECT * FROM `dhappdatabase`.`dispensation`  WHERE (`serialNumber`='$rrows[serialNumber]') and (`dataOfPickup` BETWEEN '$get[startDate]' AND '$get[endDate]' )";
										...
										$excel .= "$Code \t $patientARTCode \t $artSartDate \t $Sex \t $DoB \t $Age \t $Prof \t $Statut \t $Jan \t $Fev \t $Mar \t $Apr \t $May \t $Jun \t $Jul \t $Aug \t $Sept \t $Oct \t $Nov \t $Dec \n";													
										$li++;
									} 
	header("Content-type: application/vnd.ms-excel");
	header("Content-disposition: attachment; filename=TreatmentFollowUP-".date('Y-m-d').".xls");
	print $excel;
	exit; 
?>