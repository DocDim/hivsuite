...
//define ("DB_USER", "joomla"); // set database user
//define ("DB_PASS","joomla"); // set database password
//define ("DB_NAME","taammundebd2"); // set database name
//***************************************************************
//$link = mysql_connect("localhost","root","upecPa55") or die("erreur de connexion");
//$db = mysql_select_db("dhappdatabase");
...
define ("ME_LEVEL", 4);
define ("DATA_CLERK_LEVEL", 3);
define ("CLINICIEN_LEVEL", 2);
define ("PHARMACIEN_LEVEL", 1);
define ("NURSE_LEVEL", 0);
...
if (!isset($_SESSION['user_id']) && !isset($_SESSION['user_name']) ) 
{
	...
	$rs_ctime = mysqli_query($link, "select `ckey`,`ctime` from `users` where `id` ='$cookie_user_id'");
...
$sess_user_id = strip_tags(mysqli_real_escape_string($_SESSION['user_id']));
$cook_user_id = strip_tags(mysqli_real_escape_string($_COOKIE['user_id']));
...
if($_SESSION['user_level'] == DATA_CLERK_LEVEL) {
return 1;
} else { return 0 ;
}
...
function checkSuperviseur() {

if($_SESSION['user_level'] == ME_LEVEL) {
return 1;
} else { return 0 ;
}
}
...
if($_SESSION['user_level'] == CLINICIEN_LEVEL) {
return 1;
} else { return 0 ;
}
...
if($_SESSION['user_level'] == PHARMACIEN_LEVEL) {
return 1;
} else { return 0 ;
}
...
if($_SESSION['user_level'] == NURSE_LEVEL) {
return 1;
} else { return 0 ;
}
}
