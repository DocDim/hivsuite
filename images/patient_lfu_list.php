...
<html> 
<head>
<title>Liste des Défaillants</title>
		...
		<div id="main" class="row-fluid">
			...
			<div id="content" class="column span9">
			  ...
			  <div id="printableArea">
						...
						<span style="font-size: 36px;"><strong><h3 class="titlehdr">Liste des Défaillants</h3></strong></span>
						</div>	
													
					<?php 
												  
					$sql = "select * from `dhappdatabase`.`patient` where `serialNumber` in (SELECT `serialNumber` FROM `dhappdatabase`.`dispensation`  WHERE  ((TO_DAYS(DATE(NOW()))>TO_DAYS(DATE(nextPickupDate)))and (TO_DAYS(DATE(NOW()))-TO_DAYS(DATE(nextPickupDate))>90) and (`serialNumber` in (SELECT `serialNumber`  FROM (SELECT *
FROM  `dispensation`
INNER JOIN
...
AND `dispensation`.`id` = `topscore`.`Maxscore`) as t)))"; 
